...
def check_file_size(pdf_path: Path) -> Issue:
    ...
    if size_kb > 1024:
        return Issue(Issue.CRITICAL, "File too large",
                     f"{size_kb:.0f} KB — many portals reject files over 500–1024 KB")
    elif size_kb > 500:
        return Issue(Issue.WARNING, "File approaching size limit",
                     f"{size_kb:.0f} KB — some portals cap uploads at 500 KB")
    else:
        return Issue(Issue.OK, "File size is fine",
                     f"{size_kb:.0f} KB — well under typical portal limits")
...
def check_images(pdf_path: Path) -> list[Issue]:
    ...
    if large_images > 0:
        issues.append(Issue(Issue.CRITICAL, "Large images detected (likely a photo or graphic)",
    ...
    if small_images > 0:
        issues.append(Issue(Issue.WARNING, "Small images/icons detected",
    ...
    if total_images == 0:
        issues.append(Issue(Issue.OK, "No embedded images",
...
def check_text_extraction(pdf_path: Path) -> list[Issue]:
    ...
    if text_length < 50:
        issues.append(Issue(Issue.CRITICAL, "Almost no extractable text",
    ...
    if alpha_ratio < 0.4:
        issues.append(Issue(Issue.CRITICAL, "Text appears garbled",
        ...
        issues.append(Issue(Issue.WARNING, "Text may have extraction issues",
                            f"Alphabetic ratio {alpha_ratio:.0%} is lower than expected. "
                            "Some text may not parse correctly in ATS."))
    else:
        issues.append(Issue(Issue.OK, "Text extracts cleanly",
                            f"Extracted {text_length:,} characters with {alpha_ratio:.0%} "
                            "readable content."))
    ...
    issues.append(Issue(Issue.OK, "Extracted text saved",
...
def check_columns_and_tables(pdf_path: Path) -> list[Issue]:
    ...
    with pdfplumber.open(str(pdf_path)) as pdf:
        for page_num, page in enumerate(pdf.pages):
            ...
            if tables:
                issues.append(Issue(Issue.WARNING, "Table layout detected",
            ...
            if len(words) > 20:
                ...
                if len(sorted_x) > 2:
                    ...
                    gaps = [(sorted_x[i+1] - sorted_x[i], i)
                            for i in range(len(sorted_x) - 1)]
                    gaps.sort(reverse=True)
                    if gaps and gaps[0][0] > 100:  # large horizontal gap = likely 2 columns
                        issues.append(Issue(Issue.WARNING, "Multi-column layout suspected",
    ...
    if not any(i.title.startswith("Table") or i.title.startswith("Multi") for i in issues):
        issues.append(Issue(Issue.OK, "Single-column layout detected",
...
def check_sections(pdf_path: Path) -> list[Issue]:
    ...
    if missing_sections:
        ...
        issues.append(Issue(Issue.WARNING, "Missing common sections",
        ...
        issues.append(Issue(Issue.OK, "All key sections found",
                            "Detected: " + ", ".join(found_sections.keys())))

    if not has_email:
        issues.append(Issue(Issue.CRITICAL, "No email address detected",
                            "ATS needs your email to contact you. Make sure it's plain text, "
                            "not in an image or special font."))
    if not has_phone:
        issues.append(Issue(Issue.WARNING, "No phone number detected",
                             "Consider adding a phone number in plain text format."))

    if has_email and has_phone:
        issues.append(Issue(Issue.OK, "Contact info found",
                            "Email and phone detected in readable text."))
...
def check_fonts(pdf_path: Path) -> list[Issue]:
    ...
    safe_fonts = {"arial", "helvetica", "timesnewroman", "times",
                  "calibri", "garamond", "georgia", "verdana",
                  "tahoma", "trebuchet", "courier", "couriernew",
                  "dejavusans", "dejavuserif", "notosans", "roboto",
                  "liberationsans", "liberationserif"}

    # Symbol/Wingding fonts are red flags
    symbol_fonts = {"symbol", "wingdings", "zapfdingbats", "wingding",
                    "mathfont", "symbolfont"}

    # Normalize font names for comparison
    normalized_fonts = {f.lower().replace("-", "").replace(" ", "").replace("_", "")
                        for f in all_fonts}
    normalized_safe = {f.lower().replace("-", "").replace(" ", "").replace("_", "")
                        for f in safe_fonts}
    normalized_symbol = {f.lower().replace("-", "").replace(" ", "").replace("_", "")
                         for f in symbol_fonts}

    found_symbol = normalized_fonts & normalized_symbol
    found_unsafe = normalized_fonts - normalized_safe - normalized_symbol

    if found_symbol:
        issues.append(Issue(Issue.CRITICAL, "Symbol/decorative fonts detected",
    ...
    if found_unsafe:
        ...
        likely_embedded = {f for f in found_unsafe if any(
            f.startswith(p) for p in {"cambria", "consola", "cfx", "aabcio",
                                       "nimbus", "noto", "source", "freesans"})}
        unusual = found_unsafe - likely_embedded
        if unusual and len(unusual) <= 5:
            issues.append(Issue(Issue.WARNING, "Non-standard fonts detected",
                                f"Fonts: {', '.join(list(unusual)[:5])}. "
                                "Unusual fonts may not render in all ATS. "
                                "Stick to Arial, Calibri, Garamond, or Helvetica."))
        elif len(unusual) > 5:
            issues.append(Issue(Issue.WARNING, "Many non-standard fonts",
    ...
    if not found_symbol and (not found_unsafe or found_unsafe <= likely_embedded):
        issues.append(Issue(Issue.OK, "Fonts look ATS-friendly",
...
def check_metadata(pdf_path: Path) -> list[Issue]:
    ...
    found_sensitive = {k: v for k, v in metadata.items()
    ...
    for field, value in found_sensitive.items():
        ...
        if any(kw not in value.lower() for kw in ["microsoft", "adobe", "word",
                                                     "latex", "libre", "ghost",
                                                     "pdf", "writer", "chrome",
                                                     "safari", "macos"]):
    ...
    if personal_leaks:
        issues.append(Issue(Issue.WARNING, "Personal info in PDF metadata",
        ...
        issues.append(Issue(Issue.OK, "PDF metadata is clean",
...
def print_report(pdf_path: Path, issues: list[Issue]):
    ...
    print(f"{BOLD}{'═' * 60}{RESET}")
    if criticals:
        print(f"{RED}{BOLD}  ✗ NOT ATS-FRIENDLY{RESET} — {len(criticals)} critical issue(s) must be fixed")
    elif warnings:
        print(f"{YELLOW}{BOLD}  ⚠ LIKELY ATS-COMPATIBLE{RESET} — {len(warnings)} warning(s) to review")
    else:
